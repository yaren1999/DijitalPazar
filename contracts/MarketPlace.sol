...
contract Marketplace is Ownable, Pausable {
    ...
    event ItemCancelled(uint256 id);
    ...
    function listItem(uint256 _nftId, uint256 _price) public whenNotPaused {
        ...
        require(
            nftContract.getApproved(_nftId) == address(this) || nftContract.isApprovedForAll(msg.sender, address(this)),
            "Marketplace yetkili degil (Approve gerekli)"
        );
    ...
    function buyItem(uint256 _id) public whenNotPaused {
        ...
        require(item.status == Status.Active, "Urun satis icin aktif degil");
    ...
    function updateItemPrice(uint256 _id, uint256 _newPrice) public whenNotPaused {
        require(_id > 0 && _id <= itemCount, "Urun mevcut degil");
        Item storage item = items[_id];
        
        require(msg.sender == item.seller, "Sadece satici guncelleyebilir");
        require(item.status == Status.Active, "Sadece aktif urunler guncellenebilir");
    ...
    function cancelListing(uint256 _id) public whenNotPaused {
        require(_id > 0 && _id <= itemCount, "Urun mevcut degil");
        Item storage item = items[_id];
        
        require(msg.sender == item.seller, "Sadece satici iptal edebilir");
        require(item.status == Status.Active, "Urun zaten aktif degil");
